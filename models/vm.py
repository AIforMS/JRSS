...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, src, flow):
        # src: torch.Size([1, 12, 192, 160, 192]), self.grid: torch.Size([1, 3, 192, 160, 192]), 
        # flow: torch.Size([1, 24389, 225, 3, 3])
...
if __name__ == "__main__":
    size = [160, 192, 160]
    stn = SpatialTransformer(size=size)
    src = torch.randn(size=[1, 1, 160, 192, 160])
    flow = torch.randn(size=[1, 3, 160, 192, 160])
    src_ = stn(src, flow)
