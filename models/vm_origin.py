...
class VxmDense(LoadableModel):
    ...
    def forward(self, source, target, mov_img=None, mov_seg=None, bidir=False, for_reg=True):
        '''
        Parameters:
        ...
        if mov_seg is not None:
            if bidir:
                y_label = self.transformer(mov_seg, neg_flow, mode="nearest")
            else:
        ...
        if for_reg:
            ...
            if mov_seg is not None:
                return (y_target, y_label, neg_flow) if bidir else (y_source, y_label, pos_flow)
            else:
                return y_source, pos_flow
        else:
            return (y_target, preint_flow) if bidir else (y_source, preint_flow)
...
if __name__ == '__main__':
    ...
    y_source, pos_flow = net(torch.randn([1, 1, H, W, D], device=device),
                             ...
                             moving=torch.randn([1, 1, H, W, D], device=device))
