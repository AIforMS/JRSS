...
class TI_Loss(nn.Module):
    ...
    def set_kernel(self):
        ...
        self.kernel = torch.from_numpy(np.expand_dims(np.expand_dims(np_kernel, axis=0), axis=0))
...
class PatchNCELoss(nn.Module):
    def __init__(self, batch_size=1, nce_T=0.07):
...
if __name__ == "__main__":
    ...
    if dim == 2:
        ...
        x_ = torch.rand(1, num_classes, depth, height, width)
        ...
        MI_loss_value = MI_loss(x.cuda(), x_.cuda())
        print("MI_loss_value: ", MI_loss_value)

        patchnce_loss = PatchNCELoss()
        patchnce_loss_value = patchnce_loss(x, x_)
        print("patchnce_loss_value: ", patchnce_loss_value)
